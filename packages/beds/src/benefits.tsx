...
export function BenefitsSection({ title, description, items, action, note, purpose = 'section' }: BenefitsSectionProps) {
  ...
  return <Element className="es-benefits" aria-labelledby={`${id}-title`}>
    <div className="es-benefits-inner">
      <header className="es-benefits-intro">
        <Heading id={`${id}-title`}>{title}</Heading>
        {description && <p>{description}</p>}
      </header>
      {items.length > 0 && <ul className="es-benefits-grid" role="list">{items.map((item, index) => <li key={item.id} className="es-benefit" data-featured={index === 0 || undefined}>
        <article aria-labelledby={`${id}-item-${index}`}>
          {item.illustration && <div className="es-benefit-media">{item.illustration}</div>}
          <div className="es-benefit-copy">
            <CardHeading id={`${id}-item-${index}`}>{item.title}</CardHeading>
            {item.description && <p>{item.description}</p>}
          </div>
        </article>
      </li>)}</ul>}
      {(action || note) && <div className="es-benefits-footer">
        {action && <a className="es-benefits-action" href={action.href}><span>{action.label}</span><Icon name="ArrowRight" purpose="action" /></a>}
        {note && <p>{note}</p>}
      </div>}
    </div>
  </Element>;
...
function Paper({ x, y, angle = 0, highlight = false }: { x: number; y: number; angle?: number; highlight?: boolean }) {
  return <g transform={`translate(${x} ${y}) rotate(${angle} 60 82)`}>
    <rect className="es-benefit-art-paper" width="120" height="164" rx="12" />
...
export function BenefitIllustration({ kind }: { kind: BenefitIllustrationKind }) {
  ...
  return <svg className="es-benefit-art" viewBox={wide ? '0 0 600 240' : '0 0 320 240'} aria-hidden="true" focusable="false">
    ...
    {kind === 'documents' && <>
      <g transform="translate(42 76) rotate(-5 65 46)">
        <rect className="es-benefit-art-paper" width="134" height="94" rx="14" />
      ...
      <path className="es-benefit-art-connector" d="M190 120H270m-7-6 7 6-7 6" />
      <Paper x={293} y={36} angle={-7} />
      <Paper x={386} y={32} angle={6} highlight />
      <g transform="translate(505 158)"><circle className="es-benefit-art-paper" cx="0" cy="0" r="22" /><g transform="translate(-10 -10)"><Icon name="Check" purpose="feature" /></g></g>
    </>}
    {kind === 'profile' && <>
      <path className="es-benefit-art-orbit" d="M60 125a100 70 0 1 1 200 0a100 70 0 1 1-200 0" />
      <g transform="translate(88 59)"><rect className="es-benefit-art-paper" width="146" height="124" rx="16" /><circle cx="73" cy="37" r="20" fill="var(--es-subtle)" /><g transform="translate(63 27)"><Icon name="UserRound" purpose="feature" /></g><Lines x={34} y={76} width={78} rows={3} /></g>
      <g transform="translate(37 123) rotate(-8)"><rect className="es-benefit-art-paper" width="48" height="48" rx="12" /><g transform="translate(14 14)"><Icon name="FileText" purpose="feature" /></g></g>
      <g transform="translate(234 53) rotate(8)"><rect className="es-benefit-art-paper" width="48" height="48" rx="12" /><g transform="translate(14 14)"><Icon name="MessageSquare" purpose="feature" /></g></g>
    </>}
    {kind === 'match' && <>
      <g transform="translate(46 52) rotate(-4 90 65)"><rect className="es-benefit-art-paper" width="184" height="130" rx="14" />
    ...
    {kind === 'conversation' && <>
      <g transform="translate(87 52) rotate(4 84 29)"><rect className="es-benefit-art-paper" width="166" height="58" rx="16" /><Lines x={18} y={20} width={130} rows={2} /></g>
      <g transform="translate(49 109) rotate(-3 102 35)"><rect className="es-benefit-art-paper" width="206" height="77" rx="16" /><g transform="translate(18 19)"><Icon name="Sparkles" purpose="feature" /></g><Lines x={51} y={19} width={132} rows={3} /></g>
    ...
    {kind === 'board' && <g transform="translate(30 47)">
      {[0, 1, 2].map(column => <g key={column} transform={`translate(${column * 89} 0)`}>
        ...
        {Array.from({ length: column === 0 ? 2 : 1 }, (_, row) => <g key={row} transform={`translate(7 ${33 + row * 58}) rotate(${column === 1 ? -5 : 0} 33 25)`}>
          <rect className="es-benefit-art-paper" width="66" height="48" rx="7" /><Lines x={10} y={12} width={46} rows={2} />
