...
import { suppressThemeTransitions } from './lib/suppress-theme-transitions';
...
export function DesignSystemProvider({ children, theme, brandColor, onThemeChange }: {
  ...
  useLayoutEffect(() => {
    ...
    if (root.current) return suppressThemeTransitions(root.current);
  }, [theme]);
...
/**
 * Displays caller-owned identity artwork; BEDS supplies no product logo.
 * Use a native image in the app for a wordmark or a differently sized composition.
 * Before composing UI, read ../docs/INTERFACE-QUALITY.md and its linked SKILL.md files.
 */
export function BrandMark({ src, label }: { src: string; label: string }) {
  if (!src?.trim()) throw new Error('BrandMark requires a caller-owned image src.');
  if (!label?.trim()) throw new Error('BrandMark requires an accessible label.');
  return <img className="es-brand-mark" src={src} alt={label} />;
}
