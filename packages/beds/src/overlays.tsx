...
export function DropdownMenu({ label, icon = 'MoreHorizontal', open, onOpenChange, items, onSelect }: {
  ...
  return <div className="es-dropdown"><IconButton ref={anchor} label={label} icon={icon} aria-haspopup="menu" aria-expanded={open} aria-controls={open ? id : undefined} onClick={() => { resetTypeahead(); setActiveId(null); onOpenChange(!open); }} onKeyDown={event => { if (event.key === 'ArrowDown' || event.key === 'ArrowUp') { event.preventDefault(); resetTypeahead(); setActiveId(nextOption(items, undefined, event.key === 'ArrowUp' ? 'End' : 'Home')); onOpenChange(true); } }} />
