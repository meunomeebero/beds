...
Version `0.2.0-rc.6` consolidates the breaking, product-agnostic migration on `main`. It requires a deliberate `brandColor`, ships `beds/manifest.json` for agents and adds `HandDrawnArrow`, `ExpandingButton`, `LinkButton` and the `nav` purpose for `TextLink`. Screens are not done until they pass the Personality layers in Foundations. Use the immutable archive and SHA-256 from the [release](https://github.com/meunomeebero/beds/releases/tag/v0.2.0-rc.6). Agents start with [Agent quickstart](docs/AGENT-QUICKSTART.md). Follow [Agnostic DS](docs/AGNOSTIC-DS.md) and [API boundaries](docs/API-BOUNDARIES.md). Do not upgrade existing consumers implicitly. Historical 0.1 releases expose a different API.
