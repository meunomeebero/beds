...
| Segments |Named options → controlled exclusive value |Native pointer/Space activation; pill Left/Right and joined Left/Right/Up/Down/Home/End move focus while skipping disabled options; focus may move before a controlled callback is accepted, but checked/indicator state never anticipates `value`; selected surface uses instance-scoped layout motion and becomes instant under reduced motion; long labels remain in a contained horizontal lane, focused choices are revealed with nearest inline scroll, and reduced motion makes that reveal instant without page/document overflow |
...
| InputOTP |Labelled 4/6/8 digit value → editing/paste/autofill → caller verification state |Controlled `value`/`onChange`; native one-time-code and numeric input; spaces, hyphens and non-digits normalized on paste; `onComplete` fires only on incomplete → complete; processing/success read-only; error editable and recoverable; message and ARIA state remain caller-owned; status never implies verification |
...
- CommandPalette keeps native modal focus/scroll-lock semantics through the shared `lib/modal` helper; beUI fuzzy cursor and active-row motion are adapted, while PresenceGate/panel entrance motion is omitted to keep measured dialog geometry stable. ArrowUp/Down cycle enabled options; Home/End preserve the platform-native field behavior in the focused field. Reduced motion preserves the same keyboard and ARIA state without transform motion.
- InputOTP adapts beUI `otp-input` digit/caret/error/success intent onto the existing `input-otp` native-input foundation: digit enter/exit uses the BEDS easing, the caret blinks linearly, error shakes once, and success uses an inline BEDS `Check` icon with a 500/28 spring. `useReducedMotion` settles every visual state immediately; slot geometry and caller-owned message/status semantics do not move. Upstream SVG path drawing, 100ms delay and any unverified `layoutId` ring are omitted.
...
| Control motion |Migrated controls carry interaction motion in `motion/react` and trivial color hover in Tailwind `transition-colors` (150ms BEDS easing) — never CSS in `controls.css`, which is not on the `check-library.mjs` allowlist. Button/IconButton/IconToggleButton press → `scale(.97)` in 120ms (transient; nothing measures mid-tap); Button hover → non-geometric — filled variants lift via `filter: brightness(1.06)`, ghost keeps its `hover:bg-accent` token veil — gated by `useHoverCapable` so touch never sticks a phantom hover, and layout-measured boxes never scale on hover (a geometric hover scale left a residual transform that `boundingBox()` measured); Checkbox check scales `.6→1` in 160ms; Switch thumb travels `motion.x` → 14px in 180ms; a newly invalid field shakes once (`x:[0,-3,3,-2,2,0]`, 280ms) and its error settles in via AnimatePresence (180ms); SegmentedControl selected pill/joined surface glides through an instance-scoped `layoutId` with `SPRING_LAYOUT` on pointer selection, each choice preserves beUI press scale `.92`, while keyboard selection/reveal is instant and direction-aware in RTL; the focus perimeter is painted inside the compact lane and forced-colors uses `Highlight`; the controlled `value` remains the only checked/indicator signal; RadioGroup default selected dot glides only after pointer acceptance through an instance-scoped `layoutId` with `SPRING_LAYOUT`, default pointer press is `.92`, keyboard Arrow/Home/End/Space/Tab is instant, and question rows have no transform/layout motion; activity/connection Tabs fade color on the `:hover` recolor; settings Tabs use a shared `motion.span` layout indicator with a 2px underline and reduced-motion zero-duration fallback; AppShell interpolates only its measured 264px/62px grid columns and NavItem uses a scoped active-row layout indicator. CSS `transition`/`@keyframes` survive only in the shrinking allowlist of unmigrated stylesheets in `check-library.mjs` — legacy idiom still live there: NavItem color/background 150ms (`layout.css`). All of it collapses under `prefers-reduced-motion:reduce` (`useReducedMotion` on every motion usage; `motion-reduce:transition-none` on migrated color transitions); motion never replaces the non-animated state signal |
