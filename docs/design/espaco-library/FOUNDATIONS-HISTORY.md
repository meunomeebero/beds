# Historical foundations — not the current style guide

Archived during the product-agnostic migration. This preserves former decisions,
measurements and instructions for provenance; imperative wording below is not
active policy. Use [Foundations](FOUNDATIONS.md) and [Agnostic DS](AGNOSTIC-DS.md)
for current ownership and guidance. Do not reintroduce product layouts, logos,
consumer-CSS bans or old dimensions merely because this archive records them.

Migration notice: [Agnostic DS](AGNOSTIC-DS.md) is the current ownership decision.
The product-specific page measurements below are legacy implementation/provenance,
not a requirement to reproduce those compositions in another app. Runtime token
values remain stable while individual components are reviewed. BEDS no longer
provides product logos or named product brand presets.
...
|---|---|
| `--es-brand` |Validated provider `brandColor`; default`#d0f300`; application-owned brand color, no product presets |
