...
The source review adopted the compatible motion intent from beUI `otp-input` without importing beUI at runtime. Digit enter/exit is `14px` vertical travel with opacity and blur over `220ms` using BEDS `EASE_OUT`; the caret blinks with a `1s` linear cycle; a newly entered error shakes the group through `[0,-5,5,-3,3,-1,0]` over `450ms`; success mounts the BEDS `Check` icon inline in `.es-otp-message` from `scale(.6)`/opacity `0` to `scale(1)`/opacity `1` with a `500/28` spring. `useReducedMotion` settles these states immediately and keeps focus, status text and slot geometry unchanged.
...
| Processing/success/error semantics | same spec — main flow and forced-colors case | PASS: read-only/busy, success icon/message, editable error and non-color status verified | Native AT announcement remains pending |
| Intermediate and settled motion | same spec — `OTP motion has intermediate and settled frames` | PASS: digit transform/filter/opacity and error transform are observed during and after settling | Exact frame timing can vary by browser scheduler |
...
| Light/dark/forced colors evidence | `evidence/otp/desktop-light.png`, `desktop-dark.png`, `mobile-light.png`, `mobile-dark.png`; forced-colors Playwright case | PASS: four screenshots and forced-colors structural checks are retained | Aesthetic sign-off and non-Chromium screenshots remain pending |
| SSR and package boundary | package typecheck/build/verify plus no beUI import in source | PASS: isolated worktree `npm run verify` completed; implementation uses the client boundary and existing package dependencies | The clean HEAD worktree lacked two historical drawer PNGs referenced by pre-existing docs, so verify used a temporary symlink to the preserved primary checkout evidence; no historical asset was copied or committed |
