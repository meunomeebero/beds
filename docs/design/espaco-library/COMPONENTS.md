...
|`SegmentedControl` |`label/value/options/onChange`; options id/label/disabled?; variant pill/joined |Native exclusive radios; pointer/Space use native activation; pill Left/Right and joined Left/Right/Up/Down/Home/End move focus while skipping disabled options; checked state and selected indicator remain caller-controlled under lag/rejection; selected pill/joined surface uses isolated `layoutId` motion with reduced-motion fallback; long labels use a contained inline scroll lane that reveals the focused choice (smooth only when motion is allowed, instant under reduced motion) without widening the document; pill24px group versus source MCP joined36px; fixed contextual geometry |
