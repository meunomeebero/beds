...
| Component | Contract | Evidence / constraint |
|---|---|---|
|`DesignSystemProvider` |`children`, required `theme` and `brandColor`; optional `onThemeChange` |Fixed light/dark roles; single validated brand; no palette/font/style object |
...
|`TextLink` |`href`; optional `external/ariaLabel/purpose` |Text-level native link. `purpose="inline"` (default, prose) keeps an underline from the border token. `purpose="nav"` (header, footer, link lists) has no underline at rest and draws a 1.5px `currentColor` underline from the inline start on fine-pointer hover or `:focus-visible`, 220ms, instant under reduced motion, `text-decoration` under forced colors. `external` adds a 12px up-right glyph and safe `rel`; no color/style overrides |
|`LinkButton` |`label/onClick`; optional `purpose/disabled` |Native `<button type="button">` that looks like `TextLink`, for in-page actions (open a dialog, switch a view) placed among links where a filled or ghost `Button` would compete with the screen's primary CTA. Same `inline`/`nav` purposes and focus ring; disabled never draws the underline. Navigation to a URL uses `TextLink` |
...
|`ExpandingButton` |`label`; optional onClick/type/icon=ArrowRight/mark/context=default/accessibleLabel/disabled |Pill CTA on `--es-primary`; chip and fill on `--es-brand`/`--es-on-brand`; fill grows on fine-pointer hover or `:focus-visible` with `SPRING_LAYOUT`, instant under reduced motion; press 0.97; `default` 40px (44px coarse) or `hero` 48px; `mark` is a decorative `currentColor` glyph sized 14/16px; hidden duplicate label in the fill; forced colors hide the fill. No color, width or style escape. [ExpandingButton](EXPANDING-BUTTON.md) |
|`HandDrawnArrow` |optional label/direction/shape/context/tone/drawIn |Decorative pen stroke pointing at a nearby target; three fixed shapes (curve/loop/straight), six directions, compact/default/hero geometry; note in `--font-hand`, stroke `aria-hidden`, whole component `pointer-events:none`; `brand` tints only the stroke via `--es-brand-ink`; one-time draw-in on view, full stroke under reduced motion and print. No path, color or size escape. [HandDrawnArrow](HAND-DRAWN-ARROW.md) |
...
|`SegmentedMeter` |`label/value`; max100 default; tone neutral/brand/success; optional `valueText` |28 vertical segments,16px high,2px gap/radius;shared internal anatomy with ApplicationCard;label/ARIA value authoritative;generic quantization nearest,score quantization floor; `valueText` is forwarded as `aria-valuetext` only while the finite ratio is available and is omitted when unavailable |
...
|`PagedCarousel` |`label/previousLabel/nextLabel/slideLabel({index,count})/children` |Finite manual gallery;0/1 slide stays static without Tab stop, controls or live position;2+ slides use one full-width slide per snap, drag/touch and adjacent buttons;RTL preserves visual Previous/Next order and Arrow keys follow visual direction;Left/Right on the focused region jump without animation;reduced motion makes button navigation instant;no autoplay,loop,plugins or visual escape hatch |
...
`NumberTicker` (beUI `number-ticker`, 2026-09-18) renders slot-machine rolling digits with staggered entrance; inherits font metrics; reduced-motion guards. Non-finite input stops before rounding, formatting, padding, glyph derivation or timers and renders one static `—` without prefix/suffix. Use for live metrics/counts only — never for the fabricated ATS score, which stays static by invariant. Public `className`/`digitClassName` props were removed per the BEDS contract.
