...
function ProcessingDemo({ kind, onExit, onResult }: { kind: ProcessingDemoKind; onExit: () => void; onResult: () => void }) {
  ...
  return <Stack gap="section">
    <ProcessingView title={state === 'success' ? fixture.resultTitle : fixture.title} description={fixture.description}
      ...
      scores={clock.presentation >= fixture.durationSeconds * .67 || state === 'success' ? { label: 'Indicadores ilustrativos', note: 'Dados fictícios para visualizar as barras. Não são uma previsão do seu resultado.', items: fixture.scores.map((score, index) => ({ ...score, value: state === 'success' ? score.value : score.value * Math.min(1, Math.max(0, (clock.presentation - fixture.durationSeconds * .67 - .5 - index * .5) / 2.5)) })) } : undefined}
