...
import {
  AccountMenu, Avatar, Badge, BrandMark, Breadcrumbs, Button, ChatComposer, ChatMessage, Checkbox, ContentHeader,
  ...
  PageHeader, PlanCard, ProgressBar, RecentItem, SearchField, SectionHeader, SegmentedControl, SegmentedMeter, Select, SettingsGroup, SettingsRow,
...
const textExamples: { variant: TextVariant; label: string }[] = [
  ...
  { variant: 'caption', label: 'Caption · 12 / 16' }, { variant: 'option', label: 'Option · 13 / 16' },
];
...
function ComponentsView({ announce, ber10, ber11, theme }: { announce: (message: string) => void; ber10?: boolean; ber11?: boolean; theme: Theme }) {
  ...
  return <div className="esl-document"><Stack gap="section">
    ...
    <section><Stack><SectionHeader title="Fundação e estrutura" description="Inter, hierarquia compacta, ícones por função e uma única cor de marca configurável." />
      ...
      <Demo title="PageContentHeader · receita de app" description="Exemplo opcional, não exportado por beds. O app define margens e largura."><PageContentHeader title="Identidade da página" description="Cabeçalho medido para uma tela com título, contexto e ação." leading={<Icon name="Plug" purpose="feature" />} actions={<Button label="Ação" compact onClick={() => announce('Ação de cabeçalho demonstrada.')} />} /></Demo>
...
function TokensView({ theme }: { theme: Theme }) {
  return <div className="esl-document"><Stack gap="section"><PageHeader title="Tokens" description="Tokens dos componentes. O aplicativo define sua identidade e composição; estes valores não impõem um layout de página." />
...
export default function Catalog() {
  ...
  const [view, setView] = useState<View>(initialView === 'chat' || initialView === 'components' || initialView === 'tokens' || initialView === 'feature-card' || initialView === 'empty-state' || initialView === 'decisions' || initialView === 'pricing' || initialView === 'records' ? initialView : 'components');
  const [theme, setTheme] = useState<Theme>(initial.get('theme') === 'light' ? 'light' : 'dark');
  const [brand, setBrand] = useState<'reference' | 'orange'>(['orange', 'curriculol'].includes(initial.get('brand') ?? '') ? 'orange' : 'reference');
  ...
  function changeBrand(next: 'reference' | 'orange') {
    setBrand(next);
  ...
  return <DesignSystemProvider theme={theme} brandColor={brand === 'orange' ? '#ffa133' : '#d0f300'} onThemeChange={changeTheme}>
    <AppShell collapsed={collapsed} onCollapsedChange={setCollapsed} mobileOpen={mobileOpen} onMobileOpenChange={setMobileOpen} contentWidth={view === 'chat' || view === 'decisions' ? 'chat' : view === 'empty-state' || view === 'pricing' || view === 'records' ? 'home' : view === 'feature-card' ? 'dashboard' : 'full'}
      sidebar={<>
        ...
        <SidebarSection purpose="primary"><NavItem label="Componentes" icon="Plug" active={view === 'components'} onClick={() => navigate('components')} /></SidebarSection>
        <SidebarSection label="Biblioteca de componentes">
        ...
        <SidebarSection label="Exemplos de app">
          <NavItem label="Chat · receita" icon="MessageSquare" active={view === 'chat'} onClick={() => navigate('chat')} />
        ...
        <SidebarFooter><NavItem label="Sobre esta demonstração" icon="CircleHelp" onClick={() => setStatus('Componentes públicos e exemplos de app são grupos distintos. Receitas mostram composições opcionais; não são regras universais nem exports de beds. Dados sintéticos.')} /></SidebarFooter>
      </>}
      header={<ContentHeader actions={<Inline gap="tight"><ThemeToggle /><Select label="Cor da marca" value={brand} options={[{ id: 'reference', label: 'Referência' }, { id: 'orange', label: 'Laranja · exemplo' }]} onChange={value => changeBrand(value === 'orange' ? 'orange' : 'reference')} /></Inline>}><Breadcrumbs items={[{ id: 'catalog', label: view === 'chat' ? 'Chat' : 'Biblioteca', href: catalogHref('chat', theme) }, { id: 'view', label: view === 'chat' ? 'Getting started' : views.find(item => item.id === view)!.label }]} /></ContentHeader>}>
      <Stack gap="section">
        {status && <Notice title="Demonstração local" description={status} onDismiss={() => setStatus('')} />}
        {view === 'chat' && <ChatLayout title="How can I help you today?" mark={<BrandMark src="/demo-brand.svg" label="Example brand" />} suggestions={<><SuggestionRow icon="Search" title="Research" description="Research competitors’ ads" onClick={() => setMessage('Research competitors’ ads')} /><SuggestionRow icon="Sparkles" title="Optimize" description="Find & fix wasted budget" onClick={() => setMessage('Find & fix wasted budget')} /><SuggestionRow icon="Globe" title="Plan" description="Draft a campaign plan" onClick={() => setMessage('Draft a campaign plan')} /></>} recent={<Stack gap="section"><SectionHeader title="Recent tasks" actions={<Button label="View more" variant="ghost" compact onClick={() => setStatus('Nenhuma tarefa real foi consultada; esta é uma composição de referência.')} />} />{messages.length ? <RecentItem title="Local conversation" description="A conversa desta demonstração." onClick={() => setStatus('Você está na conversa local mais recente.')} /> : <Text tone="secondary">No recent agent tasks yet.</Text>}</Stack>}>
