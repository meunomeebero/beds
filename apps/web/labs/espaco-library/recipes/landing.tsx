...
export function LandingPageLayout({ brandName, brandMark, homeHref, navigation, navigationLabel, menuLabel, accountLink, appearance, skipLabel, children, footer }: {
  ...
  return <div className="recipe-landing">
    <a className="recipe-landing-skip" href={`#${id}-main`}>{skipLabel}</a>
    <header className="recipe-landing-header">
      <div className="recipe-landing-header-inner">
        <a className="recipe-landing-brand" href={homeHref} aria-label={brandName}><span aria-hidden="true">{brandMark}</span><span translate="no">{brandName}</span></a>
        <nav className="recipe-landing-desktop-nav" aria-label={navigationLabel}>{navigation.map(link => <a key={link.href} href={link.href}>{link.label}</a>)}</nav>
        <details ref={mobileMenu} className="recipe-landing-mobile-nav" onKeyDown={event => { if (event.key === 'Escape') { event.preventDefault(); closeMenu(); } }}>
        ...
        <div className="recipe-landing-account">{appearance}{accountLink && <a href={accountLink.href}>{accountLink.label}</a>}</div>
      </div>
    </header>
...
export function LandingHero({ eyebrow, title, description, action, secondaryAction, note, children }: {
  ...
  const id = useId();
  return <section className="recipe-landing-hero" aria-labelledby={id}>
    <div className="recipe-landing-hero-copy">
      {eyebrow && <p className="recipe-landing-eyebrow">{eyebrow}</p>}
      <h1 id={id}>{title}</h1><p className="recipe-landing-description">{description}</p>
      <div className="recipe-landing-actions"><a className="recipe-landing-primary" href={action.href}>{action.label}<Icon name="ArrowRight" purpose="action" /></a>{secondaryAction && <a className="recipe-landing-secondary" href={secondaryAction.href}>{secondaryAction.label}<Icon name="ArrowUpRight" purpose="action" /></a>}</div>
      {note && <p className="recipe-landing-note">{note}</p>}
    </div>
    <div className="recipe-landing-hero-proof">{children}</div>
  </section>;
}
...
export function LandingSection({ id, children }: { id: string; children: ReactNode }) {
  return <div id={id} className="recipe-landing-section" tabIndex={-1}>{children}</div>;
}
...
export function ProductDemo({ label, contextLabel, context, resultLabel, tabs, value, onChange, note }: {
  ...
  const id = useId();
  return <figure className="recipe-product-demo" aria-labelledby={`${id}-label`}>
    <div className="recipe-product-demo-top"><h2 id={`${id}-label`}><Icon name="Briefcase" purpose="action" />{label}</h2><span>{note}</span></div>
    <div className="recipe-product-demo-grid">
      <div className="recipe-product-demo-context"><p className="recipe-product-demo-label">{contextLabel}</p>{context}</div>
      <div className="recipe-product-demo-result"><p className="recipe-product-demo-label">{resultLabel}</p><Tabs label={resultLabel} variant="settings" items={tabs} value={value} onChange={onChange} /></div>
    </div>
  </figure>;
}
...
export function DocumentPreview({ name, subtitle, sections, note }: {
  ...
  return <article className="recipe-document-preview">
    ...
    {note && <p className="recipe-document-preview-note"><Icon name="CheckCircle2" purpose="action" />{note}</p>}
  </article>;
}
...
export function ProcessSteps({ title, description, steps }: {
  ...
  const id = useId();
  return <section className="recipe-process-steps" aria-labelledby={id}><header><h2 id={id}>{title}</h2>{description && <p>{description}</p>}</header>
    <ol role="list">{steps.map((step, index) => <li key={step.id}><span className="recipe-process-number" aria-hidden="true">{String(index + 1).padStart(2, '0')}</span><h3>{step.title}</h3><p>{step.description}</p></li>)}</ol>
  </section>;
}
...
export function FAQSection({ title, description, items }: {
  ...
  const id = useId();
  return <section className="recipe-landing-faq" aria-labelledby={id}><header><h2 id={id}>{title}</h2>{description && <p>{description}</p>}</header>
