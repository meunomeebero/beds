...
export default function DataPatterns() {
  ...
  return <DesignSystemProvider theme={theme} brandColor="#d0f300" onThemeChange={setTheme}>
    <main className="esl-data-patterns" dir={params.get('dir') === 'rtl' ? 'rtl' : 'ltr'}>
      ...
      <HorizontalRail label="Trilho manual de exemplos">
        {railCards.map(card => <CollectionCard key={card.id} avatar={<Avatar name="Equipe de exemplo" />} identity={stress ? 'Equipe internacional de pesquisa e desenvolvimento de materiais acessíveis' : 'Equipe de exemplo'} selection={<IconToggleButton label={`Selecionar ${card.title}`} icon="Bookmark" pressed={exampleSelected} onPressedChange={setExampleSelected} />} title={card.title} metadata={<Text tone="secondary">{card.detail}</Text>} actions={<Button label="Abrir exemplo" compact onClick={() => undefined} />} />)}
      </HorizontalRail>
      ...
      <ResponsiveGrid>
        <ActivityPanel title="Atividade de exemplo" icon="Activity" purpose="history" description="Retome operações que você iniciou. Os dados deste painel são demonstrativos."><Stack gap="tight"><Text variant="label">Registro atualizado</Text><Text tone="secondary">{stress ? 'Este registro tem conteúdo longo para verificar leitura e acesso aos controles. '.repeat(18) : 'Ação local de exemplo.'}</Text>{stress && <Button label="Confirmar leitura do exemplo" onClick={() => setState({ kind: 'ready' })} />}</Stack></ActivityPanel>
